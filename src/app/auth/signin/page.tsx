...
async function signInAction(formData: FormData) {
  ...
  const redirectTo = formData.get("redirect")?.toString() || "/logs";

  const authResult = await authenticateUser(username, password);

  if (!authResult.success) {
    redirect(
      `/auth/signin?error=${encodeURIComponent(authResult.message)}&redirect=${encodeURIComponent(redirectTo)}`,
    );
  }
...
const SignIn: React.FC<SignInProps> = ({ searchParams }) => {
  const redirectTarget = searchParams?.redirect || "/logs";
        ...
        <div className="flex flex-wrap items-center">
          ...
          <div className="w-full border-stroke dark:border-strokedark xl:w-1/2 xl:border-l-2">
            <div className="w-full p-4 sm:p-12.5 xl:p-17.5">
              ...
              {errorMessage ? (
                <div className="bg-red-50 text-red-600 mb-6 rounded-md p-3 text-sm">
                  {errorMessage}
                </div>
              ) : null}
